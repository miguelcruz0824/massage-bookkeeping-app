...
const CalendarView: React.FC = () => {
  ...
  // ── Derived: service lookup helpers ───────────────────────────
  ...
  // ── When duration changes, update price from tiers ─────────────
  ...
  const ApptForm = () => (
    <div className="space-y-4">
      ...
      {/* Duration — only show if service has multiple tiers */}
        ...
        <div>
          ...
          <div className="flex gap-1 mt-1">
            <select value={form.time ? form.time.split(':')[0] : ''}
              ...
              className="flex-1 px-2 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 bg-white">
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Service Legend — only shows if services exist */}
      {services.length > 0 && (
